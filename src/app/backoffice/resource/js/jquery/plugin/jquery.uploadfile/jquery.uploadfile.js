...
(function ($) {
    ...
    $.fn.uploadFile = function (options) {
        ...
        var s = $.extend({
            ...
            dragDropStr: "<span style='font-size:13px;color:#4a4a4a'>파일을 여기로 드랙 하거나,<br>파일찾기/스토리지에서 찾기를 눌러서 첨부하세요</span>",
            abortStr: "Abort",
            cancelStr: "Cancel",
            deletelStr: "Delete",
            doneStr: "Done",
        ...
        var obj = this;
        var btnIdSeq = (new Date().getTime());
        //var uploadLabel = $('<div>' + $(this).html() + '</div>');
        // 2017.12.01 파일버튼 생성
        var customel = '<button id="btnWehagoAjaxFileUpload-'+btnIdSeq+'" type="button" class="LUX_basic_btn Image" style="width:inherit"><span class="sp_lux" style="width: 59px; height: 59px; background-position: 0 -307px">파일업로드</span></button>';
        customel += '<button type="button" class="LUX_basic_btn Image" style="width:inherit" onclick="javascript:alert(\'스토리지 업로드는 개발중입니다.\');"><span class="sp_lux" style="width: 59px; height: 59px; background-position: -68px -307px">스토리지에서찾기</span></button>';
        var uploadLabel = $('<div style="padding-top:20px;">' + customel + '</div>');
        ...
        function createCutomInputFile(obj, group, s, uploadLabel) {
        	...
        	/*
        	 * Q&A 및 공지사항등록시 첨부파일 업로드 버튼 이미지 맞추기 위해 아래와 같이 수정함.
        	 * admin 공지사항 파일추가 버튼 이미지 맞추기 위해 아래와 같이 수정함.
        	 * height 값이 38로 들어올때 파일업로드 버튼 height가 65로 선택됨.
        	 * height 값이 39로 들어올때 파일업로드 버튼 height가 69로 선택됨.
        	 * ----원본 소스---
			...
			//$("#btnWehagoAjaxFileUpload").appendTo(uploadLabel);
			// 업로드 위치 수정
			form.appendTo($("#btnWehagoAjaxFileUpload-"+btnIdSeq));
